...
Tactile interaction should be another expression of an existing decision, not a replacement for accessibility or gameplay authority. Drag-and-drop can make assembly feel physical, but stable identifiers, compatibility validation, and the shared crafting plan must still determine the result. Keyboard controls remain complete. When dense supporting information competes with the object being assembled, the component library should occupy a separate, clearly bounded surface; temporary windows should be opaque enough to prevent underlying information from reducing clarity.
